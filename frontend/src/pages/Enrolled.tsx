...
const Enrolled: React.FC = () => {
  ...
  return (
    <Box p={{ base: 4, md: 8 }}>
      <Heading fontSize="2xl" fontWeight="bold" textAlign="center" mb={6}>{t('dashboard.courses') || 'My Enrollments'}</Heading>
      {error && (
      ...
      {enrollments.filter((p:any)=>p.status==='CONFIRMED').length === 0 ? (
        ...
        <VStack align="stretch" gap={4}>
          {enrollments.filter((p:any)=>p.status==='CONFIRMED').map((en: any) => {
            const tier = en.tier || {};
            return (
